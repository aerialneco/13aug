...
// データ登録SQL作成
$stmt = $pdo->prepare('SELECT * FROM counselor_table WHERE email = :email;');
      ...
      <div class="flex flex-wrap -m-2">
        ...
        <div class="p-2 w-1/2">
          <div class="relative">
            ...
            <input type="password" id="password" name="password" value="<?= $result['password'] ?>" autocomplete="current-password" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
          </div>
        </div>
        <div class="p-2 w-1/2">
          <div class="relative">
